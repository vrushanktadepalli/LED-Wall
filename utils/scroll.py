...
def scroll_up(text: str, color=DEFAULT_COLOR, delay=0.112):
    ...
    for shift in range(padded_height - HEIGHT + 1):
        frame = [[padded[y + shift][x] for x in range(WIDTH)] for y in range(HEIGHT)]
        render_frame(frame)
        time.sleep(delay)
...
def display_text(text, color=DEFAULT_COLOR):
    ...
    if text_width < WIDTH:
        ...
        padded_bitmap = [
            [0] * left_pad + row + [0] * right_pad for row in padded_bitmap
        ]
    else:
        # If text is too wide, crop to fit the display
        padded_bitmap = [row[:WIDTH] for row in padded_bitmap]
